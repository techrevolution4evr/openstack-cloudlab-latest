...
maybe_install_packages ${PYPKGPREFIX}-certbot-apache
certbot certonly -d $NFQDN --apache --agree-tos -m "$SWAPPER_EMAIL" -n

#
# Add a simple revocation service that runs on shutdown/reboot and if
# the node is no longer allocated, certbot revoke .
#
cat <<'EOF' | tee /etc/systemd/system/tbhook.service
[Unit]
