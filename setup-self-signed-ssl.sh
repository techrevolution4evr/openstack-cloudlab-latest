...
if [ $? -eq 0 -a -n "$KEY_CONFIG" -a ! -e $KEY_CONFIG -a -e openssl-1.0.0.cnf ]; then
    ...
    echo '# For use with easy-rsa version 2.x and OpenSSL 1.1.0*' | tee -a $KEY_CONFIG
    echo '# For use with easy-rsa version 2.0 and OpenSSL 1.1.0*' | tee -a $KEY_CONFIG
fi
